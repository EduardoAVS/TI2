...
public class UsuarioDAO extends DAO {
	...
	public boolean insert(Usuario usuario) {
		...
		try {  
			...
			String sql = "INSERT INTO usuario (codigo, nome, senha, sexo) "
	...
	public Usuario get(int codigo) {
		...
		try {
	        ...
	        if(rs.next()){            
	        	 usuario = new Usuario(rs.getInt("codigo"), rs.getString("nome"), rs.getString("senha"), rs.getString("sexo").charAt(0));
	        }
	        st.close();
		} catch (Exception e) {
	...
	private List<Usuario> get(String orderBy) {	
		...
		try {
			...
			ResultSet rs = st.executeQuery(sql);	           
	        while(rs.next()) {	            	
	        	Usuario u = new Usuario(rs.getInt("codigo"), rs.getString("nome"), rs.getString("senha"), rs.getString("sexo").charAt(0));
	            usuarios.add(u);
	        }
	        st.close();
		} catch (Exception e) {
	...
	public boolean update(Usuario usuario) {
		...
		try {  
			Statement st = conexao.createStatement();
			String sql = "UPDATE usuario SET nome = '" + usuario.getLogin() + "', senha = '"  
